...
func (t *node) insert(word, httpMethod string, routeHandler *handlerFunc) {
	...
	if existNode == nil {
		createdNode := t.insertRecursively(word, t)
...
//	@param word：带插入单词
//	@param n： 节点
func (t *node) insertRecursively(word string, cur *node) *node {
	if len(word) == 0 {
		cur.isEnd = true
		if cur.children == nil {
	...
	if strings.HasPrefix(word, "/:") || strings.HasPrefix(word, "/{") {
		...
		if existNodeIndex > -1 {
			return t.insertRecursively(nextWord, cur.children[existNodeIndex])
		...
		return t.insertRecursively(nextWord, childNode)
	...
	for index, childNode := range cur.children {
		...
		return t.insertRecursively(word[commonPrefixLen:], tempNode)
...
func (t *node) search(word string) (*node, map[string]string) {
	...
	return t.searchRecursively(word, t, paramValueMap)
}
func (t *node) searchRecursively(word string, cur *node, paramValueMap map[string]string) (*node, map[string]string) {
	...
	for index, childNode := range cur.children {
		...
		return t.searchRecursively(nextWord, childNode, paramValueMap)
	}
	if existParamNodeIndex > -1 {
		...
		return t.searchRecursively(nextWord, cur.children[existParamNodeIndex], paramValueMap)
	}
